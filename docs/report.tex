...
My intended contribution is a vision-language driving policy: take an open
vision-language-action or vision-language-navigation checkpoint --- Alibaba's
Qwen-Robot suite \cite{qwenrobot} is the current example, but the argument does not
depend on which one --- and post-train it on data from this car so that it follows
natural-language goals (``go to the end of the hall and stop before the door'') rather
than a precomputed raceline. A second car, built from the same parts list, then makes
agent-to-agent experiments possible: two vehicles sharing pose and intent over the
network.

That is not what most of this year was spent on, and the gap is the point of this
report. A policy of that kind is only as good as the robot underneath it, and the robot
I inherited could not have produced usable training data at all. It could not report how
fast its wheels were turning, so an ``action'' recorded during a demonstration would have
been an uncalibrated PWM pulse. It had no inertial measurement, so there was no
proprioceptive state to condition on and no way to tell a skid from a straight line. It
could only be driven by a human standing in the same room with a wired gamepad, which
caps a dataset at the length of a USB cable and one operator's patience. And it existed
as one physical artifact with its configuration living in a single laptop, so a second
car was not a build --- it was an archaeology project.

So the work reported here is architectural, and I frame it that way: each change is
justified by what it makes possible downstream, not by the lap time it saves.
...
\begin{enumerate}
  \item \textbf{Actuation with feedback.} Rebuild the drivetrain around the VESC motor
        ...
        with measured consequences --- the label side of a demonstration.
  ...
  \item \textbf{Reproducibility as infrastructure.} Put the configuration, firmware
        state and scripts in the repository, so the second car is a clone rather than a
        rebuild.
\end{enumerate}

Perception is where the two halves meet: I trained and deployed a car detector on the
platform, which is both immediately useful for head-to-head racing and a rehearsal of the
train-offline, deploy-on-the-robot loop the pivot needs. All of it is checked into the
team repository \cite{atlasrepo}. What has \emph{not} been done is validate the
...
\section*{Stage 1: Actuation --- making the action space real}
...
notes that throttle could not be calibrated at all and that the same pulse produced
different speeds on different surfaces. Any policy trained on those labels would be
learning a mapping that does not exist. Rebuilding around the VESC is what turns the
action space into currents and speeds with measured consequences.
...
The network is a mode, not a fixed choice, because collecting varied data means driving
in varied places: the car can be its own 5\,GHz hotspot (highest bandwidth, tens of
metres), its own 2.4\,GHz hotspot (roughly double the range), or a client on an existing
mesh network, in which case it roams between nodes and the usable area becomes the mesh's
footprint. With an internet uplink --- a tethered phone or an LTE modem --- and a
WireGuard overlay, the same page works from anywhere, at 60--150\,ms of latency and a
reduced video setting. A steering trim (the car pulls right, and mechanical adjustment
did not remove it) is applied on the car to every input source and persisted across
restarts.
...
disappears --- leaving the web page happily up and the car deaf; the driver, the Ackermann
converter and the joystick node now respawn automatically. And an early version of the
gamepad bridge published its axes with the sign convention of the SDL joystick API rather
than that of ROS, so throttle and steering were mirrored: the same class of frame
...
temperatures, and accepts current commands directly. A logged demonstration is now
(observation, action, outcome) with all three in physical units --- and the same closed
loop is what let the current-mode launch fix the low-speed stall.

\paragraph{The camera is a motion sensor first.} The Orbbec's most valuable output is
its 200\,Hz inertial stream: it supplies the proprioceptive state a policy conditions on,
de-skews the lidar so scans are geometrically honest at speed, and gives the traction
governor a yaw rate. Vision is its second job.
...
\texttt{sensor\_msgs/Joy}, behind the same watchdog. Swapping a human for a model is a
change of sender, not a change of architecture --- and the safety envelope does not care
...
minutes of recording eliminated all 21 false detections --- the smallest possible version
of the lesson that on-robot data beats more internet data.
...
the failures described above. This is what makes ``build a second car'' a parts order
rather than a research project, and a second car is what makes agent-to-agent work
possible at all.
...
\begin{figure}[H]
  ...
  \caption{The intended pipeline. The platform work described above supplies the left
  box (data with real actions and proprioception, collected by anyone over the network),
  and the safety and control loops in the right box that a remote policy cannot be
  trusted to provide.}
  \label{fig:pivot}
\end{figure}
...
and are explicitly meant to be post-trained on a target platform's data. Alibaba's
Qwen-Robot suite \cite{qwenrobot} is the current instance --- a navigation model, a
manipulation model and a video world model, released as a stack --- and open
vision-language-action checkpoints such as OpenVLA \cite{openvla}, $\pi_0$
\cite{pi0} and NVIDIA's GR00T~N1 \cite{groot} occupy the same slot. The plan does not
...
action is the pair (steering angle, speed) that the Ackermann chain already accepts. The
lidar is the one input these models were not designed for; the cheap first approach is to
render each scan as a small bird's-eye image and let the vision encoder see it, with a
learned range tokenizer as the more principled second version. The training objective is
imitation on teleoperated episodes, with the raceline MPC available as a second source of
demonstrations that are smoother and more consistent than a human's.
...
an instruction string attached per episode, plus a way to mark an episode good or bad
without stopping. Second, a \emph{policy server}: the 2--4\,B-parameter models in this
class will not run at a useful rate on an Orin Nano, so inference belongs on the laptop
or a rented GPU, with the car keeping every fast loop --- watchdog, emergency braking,
traction governance, the 50\,Hz current loop --- and treating the policy as one more
source of \texttt{/cmd} messages, subject to the same timeout. Third, \emph{evaluation}:
success rate on held-out instructions in the hallway, plus offline action-prediction
error on held-out drives so the model can be compared without risking the car.

\subsection*{Two cars}

The second car is the part the \texttt{hardware/} folder was written for. With two
vehicles on one network, each publishing pose, velocity and intended waypoints --- over
ROS~2 with a Zenoh middleware, or the same small JSON messages the web pilot already
uses --- the experiments become leader--follower, shared-goal navigation, and cooperative
overtaking, and the interesting questions turn into how much bandwidth a useful ``intent''
message actually needs and how gracefully behaviour degrades as the link does. The
network work in Stage~3 is what determines how far apart the two cars can be while that
holds, which is why it belongs in this report rather than in a footnote about
convenience.

\subsection*{Honest risks}

The Orin Nano cannot host these models, so the split-inference design is a constraint,
not a preference, and it puts a network link inside the control loop --- acceptable at
hallway speeds with a watchdog, not acceptable at racing speeds. Post-training a model of
this size on a laptop GPU means parameter-efficient fine-tuning and a modest dataset;
a few hundred episodes from one hallway will produce a policy that is good at that
hallway. And every VLA result I am building on is reported by its authors on their own
platforms, mostly manipulators; whether they transfer to a fast Ackermann vehicle with a
20\,cm wheelbase is exactly the open question, which is the honest reason to attempt it.
...
Then the pivot, in the order the risk suggests: build the episode logger and collect a
first dataset while driving the hallway by hand, since that is useful regardless of which
model wins; stand up a policy server against a small checkpoint and verify the
timing and the safety envelope with a deliberately trivial policy before a real one; only
then post-train and evaluate. Build the second car from the \texttt{hardware/} folder in
parallel, because it is a parts order and a weekend rather than a research risk, and
because every agent-to-agent question needs it.
...
\texttt{hardware/RUNBOOK.md} before touching the VESC's application settings --- the
