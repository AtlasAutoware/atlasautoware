...
success stays low: \resBcNomSucc\% for the best variant, plain behaviour cloning, over three seeds.
Neither augmentation nor three rounds of DAgger improved on it, which is reported as it is. The platform also runs Ubuntu~26.04 with ROS~2 Lyrical in a container, with the
...
the TJHSST Systems Lab Slurm cluster, one Quadro RTX~8000 per seed (about 55 minutes a seed,
including all evaluations). Three variants are compared, each with three seeds. \emph{BC, clean
...
expert's action, and retrains on the aggregate. DAgger starts from the augmented model and keeps
its augmentation, so it is compared against that model first.
...
still leaves the route or touches a wall on most long tasks. Plain behaviour cloning is the best
variant, at \resBcNomSucc\% success [\resBcNomSuccLo, \resBcNomSuccHi] and \resBcNomColl\%
collisions. Augmentation made things worse on every nominal measure: \resBcaugNomSucc\% success
and \resBcaugNomColl\% collisions. It did not buy robustness either, since the augmented policy
still loses success when the camera is removed. DAgger improved on its own starting point, the
augmented model, lowering collisions from \resBcaugNomColl\% to \resDgthreeNomColl\% over three
rounds, but success moved only from \resBcaugNomSucc\% to \resDgthreeNomSucc\% (rounds 1 and 2:
\resDgoneNomSucc\% and \resDgtwoNomSucc\%) and never reached plain cloning. On this task the
missing information (where the goal is, and which turns have already been taken) matters more
than the distribution shift that DAgger corrects. The seed-to-seed spread is large: plain
cloning ranged from 5.3\% to 16.0\% success across its three seeds (standard deviation
\resBcNomSuccSd\ points), so single-seed comparisons of a few points, including the ones in the
ablation below, are within noise. These are the numbers to beat, not a claim that the policy is
ready to race.
...
expert, which is also the competition task. Run DAgger from the plain behaviour-cloning model
without augmentation, which isolates what DAgger itself contributes, and use more seeds, since
three are not enough to separate variants a few points apart. Collect real episodes with the OAK-D and the C1
